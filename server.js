...
// Confidentiality: hide phone depending on privacy settings
function publicUser(u, viewerId) {
  ...
  return { id: u.id, name: u.name, username: privacy.showUsername || isSelf ? u.username || null : null, phone, avatar: u.avatar || null };
}
...
app.post('/api/register', async (req, res) => {
  ...
  const user = {
    ...
    avatar: null, privacy: defaultPrivacy(), createdAt: Date.now()
  };
...
app.patch('/api/profile', auth, (req, res) => {
  const u = db.users.find(x => x.id === req.user.id);
  const { name, username, privacy } = req.body || {};
  if (name !== undefined) {
